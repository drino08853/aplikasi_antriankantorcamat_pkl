...
<body class="d-flex flex-column" style="background-color:<?= $data['warna_background'] ? $data['warna_background'] : '#6B5935' ?>;">
    ...
    <footer class="overflow-hidden position-absolute w-100 bottom-0 p-2" style="background-color: <?= $data['warna_primary'] ? $data['warna_primary'] : '#fff' ?>;color:<?= $data['warna_text'] ? $data['warna_text'] : '#fff' ?>;font-size:0.7rem;">
        ...
        <div class="text-center">
            copyright &copy; <?= date('Y') ?> by camatpadangselatan
        </div>
    </footer>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#jumlah-antrian').load('../../admin/antrian/get_jumlah_antrian.php');
            $('#antrian-sekarang').load('../../admin/antrian/get_antrian_sekarang.php');
            $('#antrian-selanjutnya').load('../../admin/antrian/get_antrian_selanjutnya.php');
            get_panggilan();

            // auto reload data antrian setiap 1 detik untuk menampilkan data secara realtime
            setInterval(function() {
                $('#jumlah-antrian').load('../../admin/antrian/get_jumlah_antrian.php').fadeIn("slow");
                ...
                $('#antrian-selanjutnya').load('../../admin/antrian/get_antrian_selanjutnya.php').fadeIn("slow");
                get_panggilan();
            }, 1000);
                ...
                if (queuePanggil.length > 0) {
                    queuePanggil.forEach((value, index) => {
                        if (!isPlay) {
                            ...
                            setTimeout(function() {
                                responsiveVoice.speak("Nomor Antrian, " + value.antrian + ", menuju, loket, " , "Indonesian Female", {
                                    rate: 0.9,
                                    pitch: 1,
                                    volume: 1,
                                    onend: () => {
